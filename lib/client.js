...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = [
			...
			".dsh-git-qa-btn{display:inline-flex;align-items:center;gap:4px;height:28px;padding:0 8px;border:1px solid var(--dsw-alias-border-l2);border-radius:6px;background:var(--dsw-alias-bg-base);color:var(--dsw-alias-label-secondary);cursor:pointer;transition:color .15s,background .15s,border-color .15s}",
			...
			".dsh-git-overview{display:flex;gap:16px;align-items:flex-start;flex-wrap:wrap}",
			".dsh-git-overview-info{flex:1 1 220px;min-width:0}",
			".dsh-git-overview-cal{flex:1 1 300px;min-width:0}",
			".dsh-git-overview-cal-title{font-size:11px;font-weight:600;color:var(--dsw-alias-label-secondary);margin-bottom:6px}",
		...
		const QUICK_ACTIONS = [
			{ agent: null, label: "Terminal", icon: TERMINAL_ICON },
			{ agent: "opencode", label: "OpenCode", icon: OPENCODE_ICON },
			{ agent: "claude", label: "Claude", icon: CLAUDE_ICON },
			{ agent: "codex", label: "Codex", icon: CODEX_ICON },
			{ agent: "antigravity", label: "Antigravity", icon: ANTIGRAVITY_ICON }
		];
		...
		function QuickActions({ cwd, t, busy, onOpen }) {
			if (!cwd || !canOpenLocally()) return null;
			return jsx("div", {
				className: "dsh-git-qa",
				children: QUICK_ACTIONS.map((item) => jsxs("button", {
					type: "button",
		...
		// ── 视图组件 ───────────────────────────────────────────────────
		function RepoOverview({ info, contributions, t }) {
			return jsxs("div", {
						...
						children: jsxs("div", {
							...
							children: [
									...
									children: jsxs("div", {
										...
										children: [
											...
											info.lastCommit && jsx(Fragment, { children: [
												...
												jsx("span", { className: "dsh-git-kv-v", children: info.lastCommit.shortHash + "  " + info.lastCommit.subject })
											] })
										]
									})
								}),
								jsx("div", {
									...
									children: [
										jsx("div", { className: "dsh-git-overview-cal-title", children: t("cal.title") }),
										jsx(ContributionCalendar, { contributions, t })
									]
								})
							]
						})
		...
		function GitView({ sessionId, useSessions, t }) {
			...
			/** 打开终端 / 启动代理 / Finder：POST → 提示结果（不刷新 git 数据）。 */
				...
				children: [
					jsxs("div", {
						...
						children: [
							...
							jsx(QuickActions, { cwd, t, busy, onOpen: openQuick }),
					...
					view.kind === "data" && jsxs(Fragment, { children: [
						...
						view.info.isRepo && jsxs(Fragment, { children: [
							jsx(RepoOverview, { info: view.info, contributions: view.contributions, t }),
